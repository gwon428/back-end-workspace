...
public class Hello {	// 클래스 명은 클래스 파일명과 동일해야 한다.
	...
	// 하나의 Java 애플리케이션에는 main 메소드를 포함한 클래스가 반드시 하나는 있어야 한다. (main이 있어야 실행이 되기 때문)
	public static void main(String[] args) {
		System.out.println("Hello, World!");	// Ctrl + F11 (실행 단축키)
	}
}
 ...
 * - 네트워크와 분산처리를 지원한다. (Java API : Java 가 제공하는 기능들(?))
 * - 멀티 스레드를 지원한다. (Java API) : (분산하여 과부하가 걸리지 않도록(?))
 * - 동적 로딩을 지원한다. (프로그램을 실행할 때 메모리 쪽에서(?) 유연하게 최적화시켜줌)
 * 
 * * JDK > JRE > JVM *
 * - JDK(Java Development Kit): 자바 개발 도구
 * 		= JRE + 실행파일(bin/javac.exe[컴파일: 우리가 쓰는 언어를 컴퓨터가 읽을 수 있도록 컴퓨터 언어로 변환], java.exe[자바 실행] 등)
