...
CREATE TABLE product(
    ...
    ente_id VARCHAR(50),						-- 업체 아이디 (나중에 FOREIGN KEY를 걸 것들까지 포함해서 한번에 작성할 수 있음)
    cate_code INT,								-- 카테고리 코드 
    prod_name VARCHAR(30) NOT NULL,				-- 상품명
    prod_desc TEXT,								-- 상품 설명
    price INT NOT NULL,							-- 가격
    stock INT NOT NULL,							-- 재고 수량
    prod_date DATE DEFAULT(current_date)		-- 상품 등록일
);		-- 상품
...
CREATE TABLE purchase(
    ...
    pur_status VARCHAR(10)
		CHECK(pur_status IN ('구매대기중', '구매중',
        '구매완료', '배송대기중', '배송중', '배송완료'))
        DEFAULT '구매대기중'						-- 구매 상태
);	-- 구매 내역
...
CREATE TABLE delivery(
    ...
    pur_code INT, 								-- 구매 코드
    deli_date DATE DEFAULT(current_date),		-- 배송 코드
...
CREATE TABLE swap(
    ...
    price INT,									-- 비용
    swap_status CHAR(2)
		CHECK(swap_status IN ('교환', '반품'))	-- 교환/반품
);		-- 교환/반품

CREATE TABLE coupon(
	coupon_code INT AUTO_INCREMENT PRIMARY KEY,	-- 쿠폰 코드
    coupon_name VARCHAR(30),					-- 쿠폰명
    expiry_date DATE DEFAULT (current_date),	-- 사용 기한
    id VARCHAR(50),								-- 아이디 
    prod_code INT								-- 상품코드
);		-- 쿠폰
...
-- FOREIGN KEY를 TABLE 생성 이후에 연결하는 ALTER TABLE
...
ALTER TABLE delivery ADD FOREIGN KEY (pur_code) REFERENCES product(prod_code);
